...
const app = {
    ...
    calculateCalories(ingredients) {
        return ingredients.reduce((total, ing) => {
            ...
            return total + (baseIng.kcal * (ing.amount / 100));
        }, 0);
    },
    ...
    renderCategories() {
        document.getElementById('categoriesList').innerHTML = this.currentRecipe.categories.map((cat, i) => `
            <span class="tag tag-category">${cat}<button onclick="app.removeCategory(${i})">✕</button></span>
        `).join('');
    },
    ...
    renderTags() {
        document.getElementById('tagsList').innerHTML = this.currentRecipe.tags.map((tag, i) => `
            <span class="tag tag-item">#${tag}<button onclick="app.removeTag(${i})">✕</button></span>
        `).join('');
    },
    ...
    renderIngredients() {
        ...
        list.innerHTML = this.currentRecipe.ingredients.map((ing, i) => {
            const baseIng = this.ingredients[ing.name.toLowerCase()];
            const calories = baseIng ? Math.round((ing.amount / 100) * baseIng.kcal) : 0;
            return `<div class="ingredient-item-edit">
    ...
    renderSteps() {
        document.getElementById('stepsList').innerHTML = this.currentRecipe.steps.map((step, i) => `
            <div class="step-item">
                <span class="step-number">${i + 1}.</span>
                <textarea placeholder="Opisz ten krok..." oninput="app.updateStep(${i}, this.value)" rows="2">${step}</textarea>
    ...
    async saveRecipe() {
        ...
        if (this.editingRecipe) {
            this.recipes[this.recipes.findIndex(r => r.id === this.editingRecipe.id)] = recipe;
        } else {
    ...
    downloadPDF(id) {
        ...
        const doc = new jsPDF();
        let y = 20;
        doc.setFontSize(20); doc.text(recipe.name, 20, y); y += 15;
        doc.setFontSize(10);
        ...
        doc.setFontSize(14); doc.text('Skladniki:', 20, y); y += 7;
        doc.setFontSize(10);
        recipe.ingredients.forEach(ing => {
            const baseIng = this.ingredients[ing.name.toLowerCase()];
            const calories = baseIng ? Math.round((ing.amount / 100) * baseIng.kcal) : 0;
            doc.text(`- ${ing.name}: ${ing.amount}${ing.unit} (${calories} kcal)`, 25, y); y += 6;
        });
        y += 5; doc.setFontSize(12); doc.text(`Razem: ${recipe.totalCalories} kcal`, 20, y); y += 10;
        doc.setFontSize(14); doc.text('Przygotowanie:', 20, y); y += 7;
        doc.setFontSize(10);
